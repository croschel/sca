...
function getOM(what) {
      ...
      document.location=('index.php?nomeArquivo=grafico_abastecimentos.php&codom=' + cod_om);
}
</Script>
...
<?php
	...
	echo "<center><br><t1>Gráfico de Abastecimentos</t1> <p> </center>";
...
//		$_GET['codom']=$_SESSION['codom'];
	...
	echo "<a href='index.php?nomeArquivo=grafico_abastecimentos.php&oc=".$_GET['oc']."&ano=".$_GET['ano']."&mes=".($_GET['mes']-1)."&codom=".$_GET['codom']."' style='text-decoration:none' title='Mês Anterior - $mes_a'>&nbsp;Anterior&nbsp;</a>&nbsp;
			<font size='4' color='blue'><b>- $mes / ".$_GET[ano]."</font></b> - 
			<a href='index.php?nomeArquivo=grafico_abastecimentos.php&oc=".$_GET['oc']."&ano=".$_GET['ano']."&mes=".($_GET['mes']+1)."&codom=".$_GET['codom']."' style='text-decoration:none' title='Próximo Mês - $p_mes'>&nbsp; Próximo&nbsp;</a>";	   
...
<?php
	include "conexao.php";
	$qry = "SELECT substr(a.dt,1,10) dt, ifnull(SUM( ad.qtd_abastecida ),0) adm, ifnull(SUM( op.qtd_abastecida ),0) op, ifnull(SUM( it.qtd_abastecida ),0) it, ifnull(SUM( e.qtd_abastecida ),0) e,
							 ifnull(SUM( m.qtd_abastecida ),0) m, ifnull(SUM( ap.qtd_abastecida ),0) ap, ifnull(SUM( t.qtd_abastecida ),0) t
					FROM (SELECT DATE_FORMAT( ADDDATE( '".$_GET['ano']."-".$_GET['mes']."-01', @num := @num +1 ) ,  '%Y-%m-%d' ) dt 
							...
							(SELECT substr(dt_abastecimento,1,10) dt, sum(qtd_abastecida) qtd_abastecida
							FROM  `abastecimento`
							WHERE dt_abastecimento between '".$_GET['ano']."-".$_GET['mes']."-01' and '".$_GET['ano']."-".$_GET['mes']."-31' and cod_tp_cota='1' and qtd_abastecida IS NOT NULL and cota_om='".$_GET['codom']."' GROUP BY SUBSTR( dt_abastecimento, 1, 10 ) ) ad on (a.dt = ad.dt)	left join
							(SELECT substr(dt_abastecimento,1,10) dt, sum(qtd_abastecida) qtd_abastecida
							FROM  `abastecimento`
							WHERE dt_abastecimento between '".$_GET['ano']."-".$_GET['mes']."-01' and '".$_GET['ano']."-".$_GET['mes']."-31' and cod_tp_cota='2' and qtd_abastecida IS NOT NULL and cota_om='".$_GET['codom']."' GROUP BY SUBSTR( dt_abastecimento, 1, 10 )) op on (a.dt = op.dt)	left join
							(SELECT substr(dt_abastecimento,1,10) dt, sum(qtd_abastecida) qtd_abastecida
							FROM  `abastecimento`
							WHERE dt_abastecimento between '".$_GET['ano']."-".$_GET['mes']."-01' and '".$_GET['ano']."-".$_GET['mes']."-31' and cod_tp_cota='3' and qtd_abastecida IS NOT NULL and cota_om='".$_GET['codom']."' GROUP BY SUBSTR( dt_abastecimento, 1, 10 )) it on (a.dt = it.dt)	left join
							(SELECT substr(dt_abastecimento,1,10) dt, sum(qtd_abastecida) qtd_abastecida
							FROM  `abastecimento`
							WHERE dt_abastecimento between '".$_GET['ano']."-".$_GET['mes']."-01' and '".$_GET['ano']."-".$_GET['mes']."-31' and cod_tp_cota='4' and qtd_abastecida IS NOT NULL and cota_om='".$_GET['codom']."' GROUP BY SUBSTR( dt_abastecimento, 1, 10 )) e on (a.dt = e.dt)	left join
							(SELECT substr(dt_abastecimento,1,10) dt, sum(qtd_abastecida) qtd_abastecida
							FROM  `abastecimento` 
							WHERE dt_abastecimento between '".$_GET['ano']."-".$_GET['mes']."-01' and '".$_GET['ano']."-".$_GET['mes']."-31' and cod_tp_cota='5' and qtd_abastecida IS NOT NULL and cota_om='".$_GET['codom']."' GROUP BY SUBSTR( dt_abastecimento, 1, 10 )) m on (a.dt = m.dt)	left join
							(SELECT substr(dt_abastecimento,1,10) dt, sum(qtd_abastecida) qtd_abastecida
							FROM  `abastecimento`
							WHERE dt_abastecimento between '".$_GET['ano']."-".$_GET['mes']."-01' and '".$_GET['ano']."-".$_GET['mes']."-31' and cod_tp_cota='6' and qtd_abastecida IS NOT NULL and cota_om='".$_GET['codom']."' GROUP BY SUBSTR( dt_abastecimento, 1, 10 )) ap on (a.dt = ap.dt)	left join
							(SELECT substr(dt_abastecimento,1,10) dt, sum(qtd_abastecida) qtd_abastecida
							FROM  `abastecimento`
							WHERE dt_abastecimento between '".$_GET['ano']."-".$_GET['mes']."-01' and '".$_GET['ano']."-".$_GET['mes']."-31' and cod_tp_cota='7' and qtd_abastecida IS NOT NULL and cota_om='".$_GET['codom']."' GROUP BY SUBSTR( dt_abastecimento, 1, 10 )) t on (a.dt = t.dt)								
